...
import com.haier.verticalrollingview.R;
...
public class VerticalRollingTextView extends View {
    ...
    /*动画间隔*/
    private int mAnimInterval = 4000;
    /*设置监听器,判断轮播到第几个位置的接口*/
    private getItemInterface getItemInterface;

    public void setNnmberListener(getItemInterface getItemInterface) {
    ...
    /**
     * 此处实现了一个构造方法，由于自定义了属性所以选择第二个
     * 在此处可以初始化，paint对象，以及一些自定义参数的初始化，以及动画
     *
     * @param context
     * @param attrs
     */
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mOrgOffsetY == -1) {
            ...
            Log.d("measure-->", "mOffset" + mOffset);
            mOrgOffsetY = mCurrentOffsetY = mOffset - mTextTopToAscentOffset;
            Log.d("measure-->", "mOrgOffsetY" + mOrgOffsetY);
            if (mDataSetAdapter.getItemCount() == 1 && isOne == false) {
                Log.d("true-->", "是一" + isOne);
            } else if (isOne == true) {
                Log.d("true-->", "是true");
        ...
        Log.d("size-->", "size" + mDataSetAdapter.getItemCount());


        //判断是否一条数据
        if (mDataSetAdapter.getItemCount() == 1 && isOne != true) {
            Log.d("size-->", "size" + mDataSetAdapter.getItemCount());
            stop();
        } else if (isOne == true) {
    ...
    public void run() {
        ...
        isRunning = true;
        Log.d("measure-->", "mCurrentOffsetYrun" + mCurrentOffsetY);
        Log.d("measure-->", "mTextTopToAscentOffsetrun" + mTextTopToAscentOffset);
        if (mDataSetAdapter.getItemCount() == 1 && isOne == false) {
        ...
        /*调用View.post()可以保证指定方法的执行是在之前调用的方法处理完成之后的 而不仅仅是方法调用已经返回。*/
        post(mRollingTask);
    }
    ...
    public void animationEnd() {
        ...
        confirmNextIndex();
        //3.位置复位
        Log.d("measure-->", "animationEndmCurrentOffsetY" + mCurrentOffsetY);
        Log.d("measure-->", "animationEndmmOrgOffsetY" + mCurrentOffsetY);
        mCurrentOffsetY = mOrgOffsetY;
        mAnimationEnded = true;
    }

    /*onAttachedToWindow是在第一次onDraw前调用的。也就是我们写的View在没有绘制出来时调用的，但只会调用一次和这个方法正好相反*/
    @Override
    ...
    private void confirmNextIndex() {
        ...
        if (getItemInterface != null) {
            getItemInterface.getNum(mNextIndex);
        }
    ...
    public interface getItemInterface {
    ...
    class InternalAnimation extends Animation {
        ...
        protected void applyTransformation(float interpolatedTime, Transformation t) {
            ...
            Log.d("measure-->", "startValue" + startValue);
            Log.d("measure-->", "endValue" + endValue);
            Log.d("measure-->", "mCurrentOffsetY" + mCurrentOffsetY);
